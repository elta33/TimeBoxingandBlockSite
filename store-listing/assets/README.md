...
| 스크린샷 | 1280×800 **또는** 640×400 PNG/JPEG, **최대 5장** | 최소 1장 필수 | ✅ **6장 제작, 제출 5장 확정** — 제출 순서 **1 → 3 → 6 → 4 → 5** (2번 차단화면은 보관·미제출). 파일: `screenshots/screenshot-0{1..6}-*{,.ko}.png` (EN/KO). 상세는 §3 |
...
| 6 (기능 비교표) | `screenshots/screenshot-06-comparison{,.ko}.png` | `screenshot-06-comparison{,.ko}.source.html` | **없음 — 그래픽.** slide 3처럼 실앱 캡처가 아니라 디자인 표. 3열(기능 / 일반 차단기 / FocusBox), FocusBox 열 tomato 틴트 강조. **경쟁사 실명 없이 "일반 차단기" 범주 대비**(CWS 정책·이미지). 마크 ✓(#ff6347/#e8ebf2)·✗(#4b5568)·△(#f5a623)·유료 pill. 디자인 원본: Claude Design 프로젝트 "FocusBox 비교표 슬라이드"(`FocusBox Screenshot 06 - Comparison.dc.html`) |
...
CWS는 스크린샷을 **최대 5장**만 받는다. 제작분 6장 중 **2번(차단 화면)을 빼고 06(비교표)을 넣는다.**

**제출 순서(업로드 순 = 노출 순):**

| 노출 위치 | 슬라이드 | 파일 |
|---|---|---|
| 1 | 1 (타임박스 스케줄러) | `screenshot-01-donut{,.ko}.png` |
| 2 | 3 (쇼츠/릴스 선별 차단) | `screenshot-03-strongblock{,.ko}.png` |
| 3 | 6 (기능 비교표) | `screenshot-06-comparison{,.ko}.png` |
| 4 | 4 (포모도로+PiP) | `screenshot-04-pomodoro{,.ko}.png` |
| 5 | 5 (통계) | `screenshot-05-stats{,.ko}.png` |

쐐기(선별 차단, 3번)를 앞세우고 06 비교표로 못박은 뒤 나머지 기능(포모도로·통계)을 보여주는 흐름.

- **2번(차단 화면, `screenshot-02-block{,.ko}.png`)은 제작·보관하되 스토어에는 제출하지 않는다.** 필요 시 프로모/랜딩 자산으로 재활용 가능.
- CWS는 파일명이 아니라 **업로드한 순서**대로 노출하므로, 위 순서대로 6→5장을 올리면 된다(파일명은 내용 기반이라 그대로 둔다).

> 06은 경쟁사 실명을 넣지 않는다 — "일반 차단기" 범주 대비다. CWS 브랜딩/키워드 정책 위반 소지와
> 디스리스펙트 이미지를 둘 다 피하기 위함(경쟁사 실명 비교표는 내부 기획 문서에서만).
