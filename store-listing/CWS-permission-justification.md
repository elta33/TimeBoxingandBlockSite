...
> **제출 언어 — 영문 하나면 충분:** 각 권한 텍스트박스에는 **영문 justification만 붙여넣으면 된다.** 아래에 영문/한글을 나란히 둔 것은 *둘 다 제출하라*는 뜻이 아니라, 영문을 기본 제출본으로 쓰고 한글은 원문 대조·한국어 리뷰어용 선택 fallback으로 남겨둔 것이다. CWS 심사팀은 영어 기반으로 일하므로, 한글만 내면 리뷰어가 자동번역에 의존해 기술 뉘앙스가 뭉개질 수 있다 — **영문을 직접 제공하는 것이 그 변수를 없애는 핵심**이다. (사용자에게 노출되는 Store listing 설명과 달리, 이 justification은 심사 내부용이라 로케일 병행이 불필요하다.)
...
> **✅ 최종 확정 (item 4) — 제출 시 Dashboard에서 이대로 적용:**
> - **카테고리 체크:** `Web history` **1개만** 체크. 나머지 8개 카테고리(개인식별정보·건강·금융/결제·인증정보·개인 커뮤니케이션·위치·User activity·Website content) 전부 **미체크**.
> - **인증 3종:** 3개 모두 체크(아래 목록).
> - **Data usage 서술란:** 아래 "서술 예시(영문)"를 그대로 붙여넣기.
...
- 그 외 카테고리(개인식별정보, 위치, 금융, 건강, 인증정보, 개인 커뮤니케이션, User activity, Website content 등)는 **모두 미해당(체크 안 함)** — TBB는 페이지 콘텐츠를 읽지 않고 URL만 본다.

> **왜 Web history를 체크하나 (모순 아님):** CWS의 "collect"는 흔히 *기기 밖 전송*으로 좁게 읽히고, 그 정의만 보면 아무것도 전송하지 않는 TBB는 **어떤 카테고리도 체크하지 않는** 것이 기술적으로 맞다. 그러나 폼 문구는 "collect **or use**"이고, 호스트 권한으로 URL을 실제로 읽는 확장이 Web history를 미체크하면 리뷰어에게 **과소 공시**로 비쳐 오히려 반려 리스크가 커진다(과소 공시 > 과다 공시 리스크). 그래서 **보수적으로 Web history를 체크하되, 서술란에서 "전송·저장 없이 on-device 즉시 판단 후 폐기"를 명시**해 "네트워크 통신 전무" 서술과의 표면적 충돌을 없앤다. 이 조합이 가장 방어적이다.
